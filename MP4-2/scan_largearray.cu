...
extern const int LOG_NUM_BANKS;
...
void prescanArray(unsigned int* outArray, unsigned int* inArray, unsigned int* blockSums, int numElements); //defined in scan_largearray_kernel.cu
...
//                h_data[i] = 1.0f;
    ...
    cudaEventRecord(start_h, 0); //record time between these two calls

    computeGold( reference, h_data, num_elements);

	cudaEventRecord(stop_h, 0);  //record time between these two calls
    ...
    unsigned int* bs_d = preallocBlockSums(num_elements);
    // **===-----------------------------------------------------------===**

    // Run just once to remove startup overhead for more accurate performance 
    // measurement
    prescanArray(d_odata, d_idata, bs_d, TILE_SIZE);
    ...
    prescanArray(d_odata, d_idata, bs_d, padded_num_elements);
    // **===-----------------------------------------------------------===**
